...
from gigantic_dataset.core.simgen import get_curve_parameters, get_pattern_parameters, get_object_dict_by_config, get_value_internal, get_default_value_from_global, convert_to_float_array, generate
...
def collect_input_params(wn: WaterNetworkModel, 
            config: SimConfig,
    ...
    for tune_name in tune_list:
        ...
        for param_name in param_names:
            ...
            old_records = get_old_records(obj_dict=obj_dict,
                                        wn=wn,
...
def collect_all_output_params(input_path : str | None = None, 
...
def gather_values_from_the_first_inp(config  :SimConfig, selected_params = ['demand', 'pressure'] ) -> dict[str,np.ndarray]:
    ...
    local_inp_path = f'gigantic_dataset/inputs/public/{os.path.basename(inp_path) }'
